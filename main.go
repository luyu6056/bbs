...
func main() {
	...
	option := []gnet.Option{gnet.WithLoopNum(runtime.NumCPU() * 2), gnet.WithTCPKeepAlive(time.Second * 600), gnet.WithCodec(&server.Tlscodec{}), gnet.WithReusePort(true), gnet.WithOutbuf(1024), gnet.WithTls(tlsconfig), gnet.WithMultiOut(true)}
