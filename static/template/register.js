...
function showBBRule() {
showDialog($('layer_bbrule').innerHTML, 'info', 'Discuz! Board1 网站服务条款');
...
websocket_func[CMD_MSG_WSU2_Register]=function(res){
	cache.Head=res.Head
	setHead(cache.Head)
	match=loadUserdata('login_ref').match(/tpl=([^&]+)/)
	var href=''
    if(match){
    ...
    setTimeout("window.location.href ='"+$('reg_succeedmessage').href+"';", 3000);
	$('reg_succeedmessage').href = href;
    $('.mn').hide();
    $('reg_succeed').style.display = '';
    $('reg_succeedmessage').innerHTML = '感谢您注册 '+cache.Head.Bbname+'，现在将以 '+cache.Head.Grouptitle+' 身份登录站点';
}
$('#registerform').unbind().submit(function(event) {
	...
	var obj={};
	obj.Username=$('input[name="regusername"]').val();
	obj.Passwd=$('input[name="passwd"]').val();
	obj.Email=$('input[name="regemail"]').val();
	obj.Seccode=$('input[name="regseccodeverify"]').val();
	obj.Type=GET('Type');
	obj.Openid=GET("Openid");
	obj.State=GET('State');
	obj.Access_token=GET("Access_token");
	ajax_post(WRITE_MSG_U2WS_Register(obj))
	return false;
});
...
function checkusername(id) {
	...
	websocket_func[CMD_MSG_WS2U_CheckRegister]=function(res){
		if(res.Result==1){
			$('#tip_'+id).parent().addClass('p_right')
		}else{
			switch(res.Result){
				case 0:
				errormessage(id, '用户名已经被注册');
				break;
				case -10:
				errormessage(id, '用户名包含敏感字符');
				break;
	...
	ajax_post(WRITE_MSG_U2WS_CheckRegister({Type:1,Name:username}))
}
...
function checkemail(id) {
	...
	ajax_post(WRITE_MSG_U2WS_CheckRegister({Type:2,Name:email}))
}
...
websocket_func[CMD_MSG_WS2U_BindAccount]=function(res){
	...
	$('#reg_avatar').attr('src', res.Img);
	$('#reg_nickname').text(res.Nickname);
	$('#regbind').show();
}
updateseccode('regseccodeverify',sceneReg,'click')
document.title="立即注册 - "+cache.Head.Bbname
