jQuery.noConflict();
data=["width", "450", "height", "253", "scale", "exactfit", "src", "", "id", "mycamera", "name", "mycamera", "quality", "high", "bgcolor", "#ffffff", "menu", "false", "swLiveConnect", "true", "allowScriptAccess", "always"];
...
function saveAvatar() {
    ...
    canvas.toBlob(function(blob){
        ...
        reader.onload = function(f) {
            ...
            xhr.responseType="blob",xhr.timeout=5000;
            xhr.addEventListener('load',function() {
                var r = new FileReader();
                r.readAsArrayBuffer(xhr.response);
                r.onload = function(f) {
                    result=read_msg(new Uint8Array(this.result)).msg;
                    console.log(result)
                    if(result.Result==1){
                        showDialog("更新成功","right");
                        $('#myavatar').html(get_avatar(result.Avatar,240));
                        return
                    }
                    if(result.Result==-43){
                        showDialog("刷新失败，请重新上传再试","notice");
                        return
                    }
                    if(result.Result!=1){
            ...
            var msg=token;
            msg=msg.concat(WRITE_MSG_U2WS_upload_avatar({Imgdata:new Uint8Array(this.result)}));
