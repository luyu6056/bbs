...
function checkFocus() {
	if(wysiwyg) {
		...
		try {
			editwin.focus();
			editbox.focus();
			editbox.contentWindow.document.body.focus();
		} catch(e) {
			editbox.focus();
			editbox.contentWindow.document.body.focus();
	...
	if(wysiwyg) {
		...
		if(initialized && !(BROWSER.firefox && BROWSER.firefox >= '3' || BROWSER.opera)) {
			...
			text = '<!DOCTYPE html PUBLIC "-/' + '/W3C/' + '/DTD XHTML 1.0 Transitional/' + '/EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">' +
				...
				'<link rel="stylesheet" type="text/css" href="/data/cache/style_' + STYLEID + '_wysiwyg.css" />' +
...
function insertAttachimgTag(aid) {
	if(wysiwyg) {
		...
		var txt = '[img_'+aid+']' + $('image_' + aid).src + '[/img]';
		insertText(txt, strlen(txt), 0);
	}
}
...
function showHrBox(ctrlid, boxtype) {
	if(typeof postimg_type == 'undefined') {
		...
		scriptNode.src = 'data/cache/common_postimg.js' ;
