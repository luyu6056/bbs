...
function extrafunc_atMenu() {
	...
	if(wysiwyg && EXTRAEVENT.shiftKey && EXTRAEVENT.keyCode == 50 && GET('Type')!="" && (GET('Type')==0 || GET('Type')==1 || GET('Type')==2)) {
		keyMenu('@', atMenu);
		ctlent_enable[13] = 0;
		doane(EXTRAEVENT);
		atkeypress = 1;
...
function extrafunc_atMenuKeyUp() {
	...
	if(wysiwyg && EXTRAEVENT.shiftKey && EXTRAEVENT.keyCode == 50 && GET('Type')!="" && (GET('Type')==0 || GET('Type')==1 || GET('Type')==2) && !atkeypress) {
		keyBackspace();
		keyMenu('@', atMenu);
		ctlent_enable[13] = 0;
		doane(EXTRAEVENT);
	}
	if(wysiwyg && $('at_menu') && $('at_menu').style.display == '' && GET('Type')!="" && (GET('Type')==0 || GET('Type')==1 || GET('Type')==2)) {
