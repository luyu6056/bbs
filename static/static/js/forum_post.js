...
function appendAttachDel(ids) {
	...
	ajax_post(WRITE_MSG_U2WS_delete_attach({Ids:aids}))
	
}
websocket_func[CMD_MSG_WS2U_delete_attach]=function(res){
	for(var i in res.Ids){
		$('#image_td_'+res.Ids[i]+' p a').parent().html('已删除')
	}
	if($('delattachop')) {
		$('delattachop').value = 1;
	}
...
function updateImageList(action, aids) {
	for(var i in aids){
		newthread_aids.push(aids[i][0])
		 var tdObj = getInsertTdId(imgBoxObj, 'image_td_'+aids[i][0]);
                tdObj.innerHTML='<a href="javascript:;"  id="imageattach'+aids[i][0]+'"><img src="'+aids[i][1]+'" id="image_'+aids[i][0]+'" onclick="insertAttachimgTag(\''+aids[i][0]+'\');doane(event);" width="110" cwidth="300"></a><p class="mtn mbn xi2"><a href="javascript:;" onclick="delImgAttach(\''+aids[i][0]+'\',1);return false;">删除</a></p><p class="imgf"><input type="text" class="px xg2" value="描述" onclick="this.style.display=\'none\';$(\'image_desc_'+aids[i][0]+'\').style.display=\'\';$(\'image_desc_'+aids[i][0]+'\').focus();"><input type="text" name="attachnew[\''+aids[i][0]+'\'][description]" class="px" style="display: none" id="image_desc_'+aids[i][0]+'"></p>'
	}
	ATTACHNUM['imageunused'] += aids.length;
	updateattachnum('image');
...
function addpolloption() {
	if(curoptions < cache.maxpolloptions) {
		...
		$('polloption_new').outerHTML = '<span>已达到最大投票数'+cache.maxpolloptions+'</span>';
	}
}
	...
	if(!op) {
		...
		for(i = 0;i < list.length;i++) {
			if(list[i].name.match('unused') && list[i].checked) {
				aids.push([list[i].value,list[i].src]);
			}
		}
		if(aids) {
			if(type == 'attach') {
...
function extraCheck(op) {
	if(!op && $('extra_replycredit_chk') && $('replycredit_times')) {
